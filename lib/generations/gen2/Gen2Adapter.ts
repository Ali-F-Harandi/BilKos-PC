...
export class Gen2Adapter implements IGenerationAdapter {
  ...
  typeChart = [
    ...
    // ... Simplified, placeholder for size, detailed in constants
  ];
  ...
  recalculateStats(mon: PokemonStats, baseStats: any): PokemonStats {
    ...
    // Recalculating all stats in-place using Gen 2 logic
    newMon.maxHp = calculateGen2Stat(baseStats.hp, mon.iv.hp, mon.ev.hp, mon.level, true);
    newMon.hp = newMon.maxHp; // Heal fully on edit

    newMon.attack = calculateGen2Stat(baseStats.atk, mon.iv.attack, mon.ev.attack, mon.level, false);
    newMon.defense = calculateGen2Stat(baseStats.def, mon.iv.defense, mon.ev.defense, mon.level, false);
    newMon.speed = calculateGen2Stat(baseStats.spe, mon.iv.speed, mon.ev.speed, mon.level, false);

    // GSC Special stats split
    newMon.spAtk = calculateGen2Stat(baseStats.spa, mon.iv.special, mon.ev.special, mon.level, false);
    newMon.spDef = calculateGen2Stat(baseStats.spd, mon.iv.special, mon.ev.special, mon.level, false);
    newMon.special = newMon.spAtk; // Reverse-compatible with Gen 1 visual representations

    // Recalculate shiny status based on edited DVs
    newMon.isShiny = isGen2Shiny(mon.iv.attack, mon.iv.defense, mon.iv.speed, mon.iv.special);
